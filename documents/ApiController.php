...
class ApiController extends CController
{
					...
					if($search_mode=="address"){
						if(in_array('delivery_distance',(array)$search_options)){
			 			...
			 			if(in_array('distace',$search_options)){
				 		    ...
				 		    '[distance]'=>number_format($val['distance'],1)." $pretty_unit"
				 		    ));
			 				if($distance_results_type==2){
				 				if ( $resp_lat = FunctionsV3::getDistanceBetweenPlot($lat,$lng,$val['latitude'],
				 					...
				 					$val['distance_plot'] = mobileWrapper::t("[distance]",array(
				 					  '[distance]'=>number_format($resp_lat,1)." $pretty_unit"
				 					));
				 					$distance = number_format($resp_lat,1);
				 					
					 			} else $val['distance_plot'] = $this->t("not available");
			 				}
			 			}
					}
	...
	public function actiongetRestaurantInfo()
	{
		...
		if($merchant_id>0){
			...
			if ($res = FunctionsV3::getMerchantInfo($merchant_id)){
				...
				$data['goftino']=  getOption($merchant_id,'merchant_extenal');
	...
	{		
        ...
        $catlist=itemWrapper::getMerchantCategory($merchant_id);
...
//        error_reporting(E_ALL);
		...
		if ($res=Yii::app()->functions->getItemById($this->data['item_id'])){
            ...
            $Items = itemWrapper::getItemByCategory($merchant_id,$this->data['cat_id'],true,rand(1,3),3,"");
			$this->code = 1;
			$this->msg = "OK";
			$this->details = array(
	...
	{				
		...
		if($res){
			...
			if ($code==1){
               ...
               $pts_redeem_amt = $details['total']['pts_redeem_amt'];
    	       $cart_error=array();
	...
	{						
		...
		try {
			...
			$params = array(
			  ...
			  'delivery_fee'=>(int)$resp['delivery_fee'],
			  'min_delivery_order'=>(int)$resp['min_order'],
	...
	{		
		...
		if($res = Yii::app()->db->createCommand($stmt)->queryAll()){
			...
			foreach ($res as $val) {		
				...
				$val['logo']=mobileWrapper::getImage($val['logo']);
				$val['rEV']=$val['status_raw']."-".$website_review_type."-".$review_baseon_status;
...
//				}
		...
		if($order_id>0){
			...
			if ( $data = mobileWrapper::getReceiptByID($order_id)){
				...
				if ( $json_details !=false){
			        ...
			        $trn_date=FunctionsV3::prettyDateTime($data['date_created']);
			        ...
			        switch ($data['trans_type']) {
        	         	case "delivery":
        	         		...
        	         		if (isset($data['delivery_date'])){
				           	   $date = FunctionsV3::prettyDateTime($data['delivery_date']);
	...
	public function actionGetUserinfo()
		...
		if ($client_id = $this->checkToken()){
			if($res = mobileWrapper::getCustomerByToken($this->data['user_token'])){
                ...
                $data['points']= PointsProgram::getTotalEarnPoints($client_id);
				$this->code = 1;
				$this->msg = "ok";
				$this->details = array(
				  'data'=>$data
    ...
    public function actionShakeList()
	{
		if (!FunctionsV3::hasModuleAddon("pointsprogram")){
			$this->code = 6;
			$this->msg = $this->t("Points not available");
    ...
    public function actionShakeGet()
	{
		if (!FunctionsV3::hasModuleAddon("pointsprogram")){
			$this->code = 6;
			$this->msg = $this->t("Points not available");
		...
		if ($client_id = $this->checkToken()){
	    	...
	    	$total_expiring_pts = PointsProgram::getExpiringPoints($client_id);
	    	$total_expenses = mobileWrapper::pointsTotalExpenses($client_id);
            $s_shake=(int)Yii::app()->functions->getOptionAdmin('s_shake');
            $min_shake=(int)Yii::app()->functions->getOptionAdmin('min_shake');
            $max_shake=(int)Yii::app()->functions->getOptionAdmin('max_shake');


            $sql="SELECT * FROM {{shake}} WHERE client_id=$client_id and  DATE(`created_at`) = CURDATE()";
            $db=new DbExt();
            ...
            if(count($res)>2){
                $point=0;
                $pointmsg=$this->t("you got you point for today");
            }else{
                $point=rand(1,100);
                $gpoint=rand($min_shake,$max_shake);

                $data["point"]=$point;
                $data["gspoint"]=$s_shake;
                $data["gpoint"]=$gpoint;
                $data["gpoint"]=$min_shake;
                $data["g23point"]=$max_shake;
                ...
                $pointmsg=$this->t("you get point");

            $dbp=new DbExt();
            $params=array(
                'client_id'=>$client_id,
                'total_points_earn'=>$point,
                'date_created'=>FunctionsV3::dateNow(),
                'ip_address'=>$_SERVER['REMOTE_ADDR'],
                'status'=>"active",
                'trans_type'=>"Shake");
...
//			}
	...
	public function actionfoodPromo()
	{
        ...
        if($res = Yii::app()->db->createCommand($stmt)->queryAll()){
        	...
        	foreach ($res as $val) {
                ...
                $merchant_info=FunctionsV3::getMerchantById($val['merchant_id']);
                $val['restaurant_name']=clearString($merchant_info['restaurant_name']);
    ...
    public function actionchangePasswordBySMSold()
    {
...
//                                $this->msg = "OK";
                            ...
                            case "pending":
                                ...
                                $this->details['next_step']='page_settings';
                                if(isset($this->details['next_step'])){
                                    $this->details['contact_phone']=$res['contact_phone'];
                                    $this->details['customer_token']=$res['token'];
                                    $this->details['email_address']=$res['email_address'];
